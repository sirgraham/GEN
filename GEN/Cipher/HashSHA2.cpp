	...
	switch(typesha2)
		{
			...
			case HASHSHA2TYPE_224		 : Sha2_224(input,size,digest);
																 result->Add(digest,	HASHSHA2_224_DIGEST_SIZE);
																 break;

			case HASHSHA2TYPE_256		 : Sha2_256(input,size,digest);
																 result->Add(digest,	HASHSHA2_256_DIGEST_SIZE);
																 break;

			case HASHSHA2TYPE_384		 : Sha2_384(input,size,digest);
																 result->Add(digest,	HASHSHA2_384_DIGEST_SIZE);
																 break;

			case HASHSHA2TYPE_512		 : Sha2_512(input,size,digest);
