...
bool XBUFFER::Insert(XWORD data, int frompos)
{
	...
	return Insert((XBYTE*)&data, sizeof(XWORD));	
}
...
{	
	...
	return Insert((XBYTE*)&data, sizeof(XDWORD));	
}
...
{	
	...
	return Insert((XBYTE*)&data, sizeof(XQWORD));	
}
...
bool XBUFFER::Resize(XDWORD newsize, bool setblocked)
{
	...
	if(buffer)
		{
			memcpy(newbuffer , buffer, (size>newsize)?newsize:size);				
			Delete(false);			
		}
			...
			switch(mask->Get()[c])
				{
				  ...
				  case 'F': { float* fvar = va_arg(arg, float*);											
