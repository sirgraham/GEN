...
DIOANDROIDSTREAMUDP::DIOANDROIDSTREAMUDP() : DIOSTREAMUDP() , XFSMACHINE(0)
{
	...
	AddState(	DIOANDROIDUDPFSMSTATE_NONE								, 								
				 		DIOANDROIDUDPFSMEVENT_GETTINGCONNEXION		,	DIOANDROIDUDPFSMSTATE_GETTINGCONNEXION,	
	...
	AddState(	DIOANDROIDUDPFSMSTATE_GETTINGCONNEXION		,
						DIOANDROIDUDPFSMEVENT_CONNECTED						,	DIOANDROIDUDPFSMSTATE_CONNECTED			 	 ,	
	...
	threadconnexion = CREATEXTHREAD(XTHREADGROUPID_DIOSTREAMUDP, __L("DIOANDROIDSTREAMUDP::DIOANDROIDSTREAMUDP"), ThreadRunFunction, (void*)this);
}
...
{	
	...
	SetEvent(DIOANDROIDUDPFSMEVENT_GETTINGCONNEXION);
			...
			switch(diostream->GetCurrentState())
				{
					...
					case DIOANDROIDUDPFSMSTATE_GETTINGCONNEXION 	: switch(diostream->IsReadyConnect(diostream->handle))
																													{
					...
					switch(diostream->GetCurrentState())
						{
							...
							case DIOANDROIDUDPFSMSTATE_GETTINGCONNEXION 	: { struct sockaddr_in loc_addr = { 0 };																																	
							...
							case DIOANDROIDUDPFSMSTATE_CONNECTED				  : { DIOSTREAMXEVENT xevent(diostream,DIOSTREAMXEVENTTYPE_CONNECTED);								
							...
							case DIOANDROIDUDPFSMSTATE_DISCONNECTING			: { DIOSTREAMXEVENT xevent(diostream,DIOSTREAMXEVENTTYPE_DISCONNECTED);								
																															xevent.SetDIOStream(diostream);		
																															diostream->PostEvent(&xevent);
